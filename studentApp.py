...
from mongo import authenticateStudent, signInsertStudentData
...
def loadModels(schoolName, cls, sub):
    ...
    system_prompt = '''
You are chatbot, who is going to help in what ever the human needs. You need to act like a human and greet them only when they do. \
Initially if the human greet, you need to greet them back. Ignore the context whenever the person is greeting you or making a general conversation \
which is irrelevant from the given context.
    '''
    ...
    vector_store = FAISS.load_local(f"./vectorDBS/{schoolName}_class_{cls}_{sub}", embedding_model)
...
def mainPage():
    with main:
        ...
        if sub:
            ...
            if os.path.exists(f"./vectorDBS/{schoolName}_class_{cls}_{sub}"):
