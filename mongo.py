...
embed = VoyageEmbeddings(voyage_api_key="pa-C_1gQndF0fx7QpliJB16WvpiB55Sv0j_Jvl2yjrc3CA", model = "voyage-lite-02-instruct", show_progress_bar=True)
@cache
def connectMongo():
    try:
...
def signInsertStudentData(username, password, teacherClass, school, subject):
   db = connectMongo()
   if db == None:
      return "error"
   else:
      try:
         db.studentData.insert_one({"username": username, "password": password, "class":teacherClass, "schoolName": school, "subject": subject})
      except Exception as e:
         return e


def signInsertTeacherData(username, password, teacherClass, school, subject):
   db = connectMongo()
   if db == None:
      return "error"
   else:
      try:
         db.teacherData.insert_one({"username": username, "password": password, "class":teacherClass, "schoolName": school, "subject": subject})
      except Exception as e:
         return e
...
def load_vdb(schoolName, cls, sub):
    try:
        return FAISS.load_local(f"./{schoolName}_class_{cls}_{sub}", embeddings=embed)
    except Exception as e:
        print(e)
        return None
